...
class RCThermalSimulator:
    ...
    self._hvac_power(action)
        ...
        baseline_act = self.baseline_action()
        (
            _,
            baseline_power,
            _,
        ...
        savings_ratio = (
                            baseline_power - P_elec
                        ) / max(baseline_power, 1.0)

        # baseline power for auditing
        T = self.T.copy()
        ...
        r_savings = 0.5 * savings_ratio
        r_power = -0.1 * P_elec
        if self.t_step % 50 == 0:
            print(
                    f"Energy={r_energy:.3f} | "
                    f"Power={r_power:.3f} | "
                    f"Comfort={r_comfort:.3f} | "
                    f"Peak={r_peak:.3f} | "
                    f"IAQ={r_iaq:.3f} | "
                    f"Savings={r_savings:.3f}"
                )

        reward    = r_energy + r_comfort + r_peak + r_iaq + r_savings + r_power
        ...
        return (
                ...
                done,
                {
                    ...
                    "baseline_power_con": baseline_power,
                    "model_power_con": P_elec,
...
class TCDDPG:
    ...
    def update(self):
        ...
        L_critic.backward()
        nn.utils.clip_grad_norm_(self.critic.parameters(), GRAD_CLIP_CRITIC)
        ...
        L_actor.backward()
        nn.utils.clip_grad_norm_(self.actor.parameters(), GRAD_CLIP_ACTOR)
        self.opt_actor.step()
...
def log_to_database(
    ...
    row = {
        ...
        "status_air_flow_supply_in_percent":
            float(np.mean(action[5:10]) * 100),

        "status_air_flow_return_in_percent":
            float(np.mean(action[5:10]) * 100),

        "status_is_heating_required":
            int(np.mean(env.T) < 21),

        "status_is_cooling_required":
            int(np.mean(env.T) > 24),

        "baseline_power_con":
            float(info["baseline_power_con"]),

        "model_power_con":
            float(info["model_power_con"]),
    ...
    print(
    "Writing:",
    info["baseline_power_con"],
    info["model_power_con"]
)
    print(
    f"Baseline={info['baseline_power_con']:.2f} | "
    f"Model={info['model_power_con']:.2f}"
)
    db.insert_dataframe(df) 
    print("LOG_TO_DATABASE CALLED")
...
def train(
    ...
    for episode in range(1, n_episodes + 1):
        ...
        while not done:
            ...
            # Select action
            # ----------------------------------------
            a_raw, a_phys = agent.select_action(
                state,
                a_prev,
                explore=True
            )
            ...
            # ----------------------------------------
            # Write operating point to SQLite
            # ----------------------------------------
            log_to_database(
                db=db,
                env=env,
        ...
        # ----------------------------------------
        history["ep_reward"].append(
            ep_reward
        )

        history["L_critic"].append(
            ep_lc / max(ep_steps, 1)
        )

        history["L_actor"].append(
            ep_la / max(ep_steps, 1)
        )
        ...
        if episode % eval_every == 0:
            ...
            history["eval_energy"].append(
                metrics["energy_kWh"]
            )

            history["eval_comfort"].append(
                metrics["comfort_viol_h"]
            )
            ...
            print(
                ...
                f"  Lc={ep_lc/max(ep_steps,1):.4f}"
                f"  La={ep_la/max(ep_steps,1):.4f}"
            )
    ...
    print(
        f"\n[TC-DDPG] Training complete. "
        f"Best eval energy: "
        f"{best_eval_energy:.2f} kWh"
    )
